 ...
 * 1 ms. (Which makes it more useful than Unix' classic 1-second resolution
 * returned by time().) "pit" stands for "Point In Time".
 *
...
#include "lib/util.h" // DOZ(), MC_USEC_PER_MSEC
...
mc_lua_timer_now (void)
{
    ...
    current_pit += (now - last_reading) / MC_USEC_PER_MSEC;
 ...
 * the time (in msesc) till the first scheduled function.
 *
 * MC's event loop uses this function to know the maximum time it's
 * got to wait for user input.
 */
gboolean
mc_lua_has_pending_timeouts (pit_t *time_out)
{
    if (next_timeout == 0)
        return FALSE;
    ...
    if (time_out != NULL)
    {
        ...
        const pit_t now = mc_lua_timer_now ();

        *time_out = DOZ (next_timeout, now);
    }
