...
luaUI_push_widget_ex (lua_State *L, Widget *w, gboolean created_in_c, gboolean allow_abstract)
{
    ...
    if (!allow_abstract && w->scripting_class_name == NULL)
    {
        /*
    ...
    lua_newtable (L);
    luaL_setmetatable (
        L, mc_lua_ui_meta_name (w->scripting_class_name ? w->scripting_class_name : "Widget"));
                       ...
                       const char *scripting_class_name)
{
    ...
    /* A boolean (false) is used to signify a destroyed widget. See
     * mc_lua_notify_on_widget_destruction() */
    if (lua_isboolean (L, -1))
    {
        ...
        if (!allow_destroyed)
            luaL_argerror (
                L, idx,
                E_ ("A living widget was expected, but an already destroyed widget was provided"));
        return NULL;
    }
    ...
    if (w != NULL && scripting_class_name != NULL
        && !STREQ (w->scripting_class_name, scripting_class_name))
    {
