 ...
 *
 * It's independent of MC.
 *
 * This library, in capi.{c,h} (which stands, if you will, for "C API
 * extensions for Lua"), serves two purposes:
 *
 ...
 * a table or as multiple values.
 *
 * The array must end with a NULL pointer.
 *
 * Returns the amount of strings the array holds.
 */
int
...
luaMC_pushlstring_conv (lua_State *L, const char *s, size_t len, GIConv conv)
{
    ...
    buffer = g_string_new ("");
    conv_result =
        str_nconvert (conv, s, len, buffer);  // @todo: will the 'size_t -> int' cast cause trouble?
...
luaMC_enable_table_gc (lua_State *L, int index)
{
    ...
    // Re-assign the meta, in case the __gc entry was added after calling setmetatable().
 ...
 *
 * see also
 * http://stackoverflow.com/questions/3332448/treating-userdate-like-a-table-in-lua/3332972#3332972
 */
gboolean
 ...
 */
void
luaMC_register_metatable (lua_State *L, const char *tname, const luaL_Reg *l, gboolean create_index)
{
