...
int luaMC_push_argv (lua_State *L, char **argv, gboolean as_table);
const char *luaMC_tolstring (lua_State *L, int idx, size_t *len);
int luaMC_pushlstring_conv (lua_State *L, const char *s, size_t len, GIConv conv);
...
/* Older Luas */
/*
 * Alas, we can't just do "#define luaL_checki luaL_checknumber" because,
 * since these older Luas use doubles, and since mc is now compiled with
 * -Wfloat-conversion and -Wbad-function-cast, code using it will emit warnings.
 *
 * See:
 *   http://thread.gmane.org/gmane.comp.lang.lua.general/122134
 *     "Compiler warnings when dealing with huge integers"
 *   and also:
 *   http://stackoverflow.com/questions/28098822/should-we-explicitly-cast-from-luas-lual-checkinteger-et-al
 *
 * So, to get around these warnings, we define functions.
 */
...
#pragma GCC diagnostic ignored "-Wfloat-equal"
    ...
    // The following was borrowed from Lua5.2's implementation of file:seek() (see liolib.c:f_seek).
    /* This code is here just as an educational tidbit. It isn't quite useful
       because it only checks that a value is outside the 64 bits range, not
       outside double's 53 bits "safe integer" range. */
    luaL_argcheck (L, (lua_Number) integral == floating, idx, "not an integer in proper range");
