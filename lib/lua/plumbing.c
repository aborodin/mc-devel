...
static gboolean
ui_is_ready_handler (const gchar *event_group_name, const gchar *event_name, gpointer init_data,
                     gpointer data)
{
...
check_for_restart (void)
{
    ...
    {
        // The script wants us to restart.
        ...
        if (lua_gettop (Lg) == 0)
            ...
            message (D_ERROR | D_CENTER, Q_ ("DialogTitle|Lua"),
                     _ ("You may not restart Lua from a dialog, or a window, opened by Lua.\n"
...
mc_lua_system_dir (void)
{
    ...
    if (dir == NULL)
    {
        ...
        if (dir == NULL)
            dir = MC_LUA_SYSTEM_DIR;  // Defined in Makefile.am. It already has MC_LUA_API_VERSION
                                      // embedded.
    }
...
mc_lua_run_script (const char *pathname)
{
    if (luaMC_get_system_callback (Lg, "mcscript::run_script"))
    {
        ...
        if (luaMC_safe_call (Lg, 1, 1))
            return luaMC_pop_boolean (Lg) ? MC_LUA_SCRIPT_RESULT_CONTINUE
    ...
    {
        fprintf (stderr, "%s\n",
                 E_ ("Internal error: I don't know how to run scripts. The core probably wasn't "
