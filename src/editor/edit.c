...
edit_do_undo (WEdit * edit)
{
    ...
    while ((ac = edit_undo_pop_action (&edit->undo_stack)) < KEY_PRESS)
    {
        switch ((int) ac)
        {
        case STACK_BOTTOM:
        ...
        else if (ac >= MARK_2 - 2 && ac < MARK_CURS - 2)
        {
...
edit_do_redo (WEdit * edit)
{
    ...
    while ((ac = edit_redo_pop_action (&edit->redo_stack)) < KEY_PRESS)
    {
        switch ((int) ac)
        {
        case STACK_BOTTOM:
...
edit_group_undo (WEdit * edit)
{
    ...
    {
        cur_ac = edit_undo_get_prev_action (&edit->undo_stack);
        edit_do_undo (edit);
        ac = edit_undo_get_prev_action (&edit->undo_stack);
...
edit_insert_file (WEdit * edit, const vfs_path_t * filename_vpath)
{
    ...
    else
    {
        ...
        if (vertical_insertion)
        {
            ...
            {
                if (!edit->column_highlight)
                    edit_undo_push_action (&edit->undo_stack, COLUMN_OFF);
                edit->column_highlight = 1;
            }
        }
        else
        {
            ...
            if (!option_persistent_selections && edit->modified)
            {
                ...
                if (edit->column_highlight)
                    edit_undo_push_action (&edit->undo_stack, COLUMN_ON);
                edit->column_highlight = 0;
            }
           ...
           long line)
{
    ...
    edit->undo_stack.size = START_STACK_SIZE;
    edit->undo_stack.size_mask = START_STACK_SIZE - 1;
    edit->undo_stack.stack = g_malloc0 ((edit->undo_stack.size + 10) * sizeof (long));

    edit->redo_stack.size = START_STACK_SIZE;
    edit->redo_stack.size_mask = START_STACK_SIZE - 1;
    edit->redo_stack.stack = g_malloc0 ((edit->redo_stack.size + 10) * sizeof (long));
...
edit_clean (WEdit * edit)
{
    ...
    g_free (edit->undo_stack.stack);
    g_free (edit->redo_stack.stack);
...
edit_insert (WEdit * edit, int c)
{
    ...
    if (c > 32)
        edit_undo_push_action (&edit->undo_stack, BACKSPACE);
    else
        edit_undo_push_action (&edit->undo_stack, BACKSPACE_BR);
    /* update markers */
...
edit_insert_ahead (WEdit * edit, int c)
{
    ...
    /* ordinary char and not space */
    if (c > 32)
        edit_undo_push_action (&edit->undo_stack, DELCHAR);
    else
        edit_undo_push_action (&edit->undo_stack, DELCHAR_BR);
...
edit_delete (WEdit * edit, gboolean byte_delete)
{
    ...
    for (i = 1; i <= char_length; i++)
    {
        ...
        edit->buffer.size--;
        edit_undo_push_action (&edit->undo_stack, p + 256);
    }
...
edit_backspace (WEdit * edit, gboolean byte_delete)
{
    ...
    for (i = 1; i <= char_length; i++)
    {
        ...
        edit->buffer.size--;
        edit_undo_push_action (&edit->undo_stack, p);
    }
    edit_modification (edit);
    if (p == '\n')
    {
...
edit_cursor_move (WEdit * edit, off_t increment)
{
    if (increment < 0)
    {
        for (; increment < 0 && edit->buffer.curs1 != 0; increment++)
        {
            ...
            edit_undo_push_action (&edit->undo_stack, CURS_RIGHT);
    ...
    else
    {
        for (; increment > 0 && edit->buffer.curs2 != 0; increment--)
        {
            ...
            edit_undo_push_action (&edit->undo_stack, CURS_LEFT);
...
{
    edit_undo_push_action (&edit->undo_stack, MARK_1 + edit->mark1);
    edit_undo_push_action (&edit->undo_stack, MARK_2 + edit->mark2);
    edit_undo_push_action (&edit->undo_stack, MARK_CURS + edit->end_mark_curs);
}
...
{
    edit_undo_push_action (&edit->undo_stack, KEY_PRESS + edit->start_display);
    if (edit->mark2 == -1)
    {
        edit_undo_push_action (&edit->undo_stack, MARK_1 + edit->mark1);
        edit_undo_push_action (&edit->undo_stack, MARK_CURS + edit->end_mark_curs);
