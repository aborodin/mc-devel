...
static int l_nonvfs_mkdir_p (lua_State *L);
static int l_mknod (lua_State *L);
static int l_rmdir (lua_State *L);
static int l_utime (lua_State *L);
static int l_chown (lua_State *L);
static int l_getlocalcopy (lua_State *L);
static int l_ungetlocalcopy (lua_State *L);
static int l_chdir (lua_State *L);
static int l_vfs_expire (lua_State *L);
static int l_nonvfs_realpath (lua_State *L);
static int l_strerror (lua_State *L);
static int l_mkstemps (lua_State *L);
static int l_nonvfs_access (lua_State *L);
static int l_file_exists (lua_State *L);
static int l_current_vdir (lua_State *L);
static int l_current_dir (lua_State *L);
static int l_vpath_new (lua_State *L);
static int l_statbuf_new (lua_State *L);
...
static const luaMC_constReg fslib_constants[] = {
    ...
    // Errors. We export only the ones we use. End-users shouldn't need to use any.
    REGC (ENOENT),
    REGC (EXDEV),
    REGC (EINVAL),
    REGC (EACCES),
...
l_readlink (lua_State *L)
{
    ...
    vpath = luaFS_check_vpath (L, 1);
    len = mc_readlink (vpath, link_target, sizeof (link_target));

    // We don't need to do "- 1"
    if (len > 0)
    {
...
l_utime (lua_State *L)
{
    ...
    times[1].tv_nsec = luaL_opti (L, 3, now);
    times[0].tv_sec = luaL_opti (L, 4, now);
    times[0].tv_nsec = luaL_opti (L, 5, now);
#else
     ...
     * judging by SHELL's shell_utime(), it's not safe to surprise a VFS
     * class with a NULL. */
...
l_nonvfs_access (lua_State *L)
{
    ...
    result = vfs_file_is_local (vpath->vpath) ? access (vpath->vpath->str, mode)
                                              : mc_stat (vpath->vpath, &sb);
 ...
 *
 * Returns **true** if the file (which may be a directory) exists,
 * or a triad otherwise.
 *
 * [info]
 *
 * This is simply a wrapper that calls @{nonvfs_access}. The benefit of
 * using it is its self-documenting name.
 *
 * [/info]
 *
