...
l_panel_set_panelized (lua_State *L)
{
    ...
    // So that the "Panelize" indicator appears.
    widget_draw (WIDGET (panel));
...
update_view (WPanel *panel)
{
    ...
    // configure_panel_listing() does do_refresh(), which is an overkill.
    widget_draw (WIDGET (panel));
}
 ...
 *      pnl.custom_format = "half type name | size | perm | gitstatus | gitdate | gitauthor |
 * gitmessage" end)
 *
 ...
 * @{list_format|list format} to "brief" (You'll have to do this yourself, as
...
l_set_num_brief_cols (lua_State *L)
{
    ...
    panel->brief_cols = CLAMP (cols, 1, 9);

    // don't waste CPU cycles otherwise.
