...
#define WDGT_GETTER(name, var)                                                                     \
    static int l_widget_get_##name (lua_State *L)                                                  \
    {                                                                                              \
        WRect *r;                                                                                  \
        r = &luaUI_check_widget (L, 1)->rect;                                                      \
        lua_pushinteger (L, r->var);                                                               \
        return 1;                                                                                  \
    }

#define WDGT_SETTER(name, var)                                                                     \
    static int l_widget_set_##name (lua_State *L)                                                  \
    {                                                                                              \
        WRect *r;                                                                                  \
        r = &luaUI_check_widget (L, 1)->rect;                                                      \
        r->var = luaL_checkint (L, 2);                                                             \
        return 0;                                                                                  \
    }
...
static int
l_widget_init (MC_UNUSED lua_State *L)
{
...
l_widget_destroy (lua_State *L)
{
    ...
    w = luaUI_check_widget_ex (L, 1, TRUE, NULL);
    if (w != NULL)
    {
        if (w->owner != NULL)
        {
...
l_widget_command (lua_State *L)
{
    ...
    if (w->callback == NULL)
        luaL_error (L,
                    E_ ("My oh my! w->callback is NULL. I cannot :command() the widget!\nAre you "
                        "trying to :command() an Editbox from its <<load>> event?"));
 ...
 * in our core). Its use is discouraged, which is why we don't document
 * its parameters here.
 *
 * See usage examples at @{git:snippets/hotlist_right_as_enter.lua} and
 * @{git:skin-sampler.lua}.
 *
 * [/note]
 *
 * @method widget:_send_message
 * @args (msg[, parm[, sender])
 */
static int
l_widget_send_message (lua_State *L)
{
    ...
    parm = luaL_optint (L, 3, 0);

    /*
     * Ideally, below we should use lua_isnoneornil() instead of
     * ensuring this is indeed a widget (table). But otherwise the
     * following common call would fail:
     *
     *    w:_send_message(ui.MSG_KEY, tty.keyname_to_keycode 'down')
     *
     * ...because keyname_to_keycode() returns *two* values, which Lua
     * is happy to pass along.
     *
     * @todo:
     * Maybe we should instead make keyname_to_keycode() return just
     * one value (unless a flag is used).
     */
    sender = (lua_type (L, 4) == LUA_TTABLE) ? luaUI_check_widget (L, 4) : NULL;
...
btn_callback (struct WButton *button, int action)
{
    ...
    // Don't close the dialog. If the programmer wants to, she can call dlg:close().
    return 0;
}
...
{
    return WIDGET (button_new (5, 5, B_USER /* isn't used */, NORMAL_BUTTON, NULL, btn_callback));
}
...
l_button_set_type (lua_State *L)
{
    ...
    button_set_flags (btn, luaMC_checkoption (L, 2, NULL, type_names, type_values));
...
l_checkbox_set_checked (lua_State *L)
{
    ...
    chk->state = lua_toboolean (L, 2) != 0;

    /* @FIXME: MC should have a setter for this so we won't have to
       deal with stuff like this ourselves. */

    widget_draw (WIDGET (chk));
...
l_checkbox_get_text (lua_State *L)
{
    ...
    chk = LUA_TO_CHECKBOX (L, 1);
    luaMC_pushstring_and_free (L, hotkey_get_text (chk->text));
    return 1;
}
...
l_set_auto_size (lua_State *L)
{
    ...
    LUA_TO_LABEL (L, 1)->auto_adjust_cols = lua_toboolean (L, 2) != 0;
    return 0;
}
...
l_input_get_text (lua_State *L)
{
    ...
    if ((ipt->completion_flags & INPUT_COMPLETE_CD) != 0)
    {
         ...
         * code. Remove? */
        luaMC_pushstring_and_free (L, tilde_expand (input_get_ctext (ipt)));
    }
    else
        lua_pushstring (L, input_get_ctext (ipt));
...
l_input_set_text (lua_State *L)
{
    ...
    {
        if (WIDGET (ipt)->owner != NULL)
        {
            // 'init_from_history' is handled in dlg_init()
            luaL_error (L, "%s",
                        E_ ("You can only set the text property to '-1' *before* adding the widget "
                            "to a dialog."));
        }
...
l_input_set_cols (lua_State *L)
{
    ...
    w = WIDGET (LUA_TO_INPUT (L, 1));
    r = &w->rect;
    cols = luaL_checkint (L, 2);

    /* Trigger update of internal variables of input line (done by its MSG_RESIZE).
     * We follow lib/widget/quick.c's example here. */
    widget_set_size (w, r->y, r->x, r->lines, cols);
...
l_input_set_password (lua_State *L)
{
    ...
    ipt = LUA_TO_INPUT (L, 1);
    ipt->is_password = lua_toboolean (L, 2) != 0;

    widget_draw (WIDGET (ipt));
    return 0;
}
 ...
 * aware of:
 *
 * - When you select entries with the keyboard, a MSG_ACTION is fired.
 *   When you select with the mouse, *nothing* is fired (see ticket #3562).
 *
...
l_listbox_set_selected_index (lua_State *L)
{
    ...
    lst = LUA_TO_LISTBOX (L, 1);
    index = luaL_checkint (L, 2);

    listbox_set_current (lst, index - 1);
    widget_draw (WIDGET (lst));
    return 0;
}
...
l_listbox_get_selected_index (lua_State *L)
{
    ...
    lua_pushinteger (L, lst->current + 1);
    return 1;
}
...
process_list (lua_State *L, item_processor f, void *data)
{
    ...
    for (i = 1; i <= len; i++)
    {
        ...
        else if (lua_istable (L, -1))
        {
            ...
            if (lua_isstring (L, -1) == 0)
                luaL_error (L,
                            E_ ("Invalid element of list at index %d: string expected as first "
                                "element of table."),
                            i);
            ...
            luaL_error (
                L,
                E_ ("Invalid element type of list at index %d: either string or table expected."),
                i);
    }
...
{
    listbox_add_item (LISTBOX (data), LISTBOX_APPEND_AT_END, keycode, label, NULL, FALSE);
}
...
set_radios_items (WRadio *rad, const char **items, int items_count)
{
    ...
    WIDGET (rad)->rect.cols = WIDGET (rad_dummy)->rect.cols;
    WIDGET (rad)->rect.lines = WIDGET (rad_dummy)->rect.lines;
...
radios__add_to_array (void *data, const char *label, long keycode)
{
    ...
    g_ptr_array_add ((GPtrArray *) data, g_strdup (label));
}
...
l_gauge_set_value (lua_State *L)
{
    ...
    gauge_set_value (g, g->max, (int) value);
    return 0;
}
...
l_gauge_set_max (lua_State *L)
{
    ...
    gauge_set_value (g, (int) max, g->current);
    return 0;
}
...
{
    gauge_show (LUA_TO_GAUGE (L, 1), lua_toboolean (L, 2) != 0);
    return 0;
}
...
{
    LUA_TO_HLINE (L, 1)->auto_adjust_cols = lua_toboolean (L, 2) != 0;
    return 0;
}
...
init_child (void *data, void *user_data)
{
    ...
    call_widget_method (WIDGET (data), "on_init", 0, NULL);
}
...
ui_dialog_callback (Widget *w, Widget *sender, widget_msg_t msg, int parm, void *data)
{
    ...
    case MSG_INIT:
    {
        ...
        g_list_foreach (GROUP (w)->widgets, init_child, NULL);
        return MSG_HANDLED;
    }
    ...
    case MSG_POST_KEY:
    {
         ...
         * to simulate an on_change event for the Input widget, as, unlike
         * other widgets, it doesn't emit MSG_NOTIFY. */
        g_assert (GROUP (w)->current != NULL); /* Guaranteed by dlg_key_event(). */
        call_widget_method (WIDGET (GROUP (w)->current->data), "on_post_key", 0, NULL);
    ...
    case MSG_VALIDATE:
    {
        ...
        if (mc_global.midnight_shutdown)
        {
             ...
             */
            // message (D_NORMAL, "Debug", "MC is exiting. I'm closing this modaless dialog
            // anyway.");
 ...
 */
static char *
dlg_title_handler (const WDialog *dlg, MC_UNUSED const ssize_t width)
{
    char *title;
    ...
    title = g_strdup (lua_tostring (Lg, -1) != NULL ? lua_tostring (Lg, -1) : "");
    lua_pop (Lg, 1);
...
dialog_constructor (void)
{
    ...
    dlg = dlg_create (TRUE, -1, -1, 4, 4, WPOS_KEEP_DEFAULT, FALSE, dialog_colors,
                      ui_dialog_callback, NULL, NULL, NULL);
...
{
    widget_idle (WIDGET (LUA_TO_DIALOG (L, 1)), lua_toboolean (L, 2) != 0);
...
l_dialog_command (lua_State *L)
{
     ...
     */
    lua_pushboolean (L,
                     send_message (dlg, NULL, MSG_ACTION, cmd, NULL) == MSG_HANDLED
...
l_dialog_set_colorset (lua_State *L)
{
    ...
    dlg->colors = luaMC_checkoption (L, 2, NULL, colorset_names, colorset_values);

    widget_draw (WIDGET (dlg));  // In case the user changes the colorset of an active dialog.
...
l_dialog_get_mapped_children (lua_State *L)
{
    ...
    g = GROUP (LUA_TO_DIALOG (L, 1));

    lua_newtable (L);
    g_list_foreach (g->widgets, (GFunc) add_child, L);
...
l_dialog_get_current (lua_State *L)
{
    ...
    /* Note the use of the _ex() version: We also push widgets that don't
       have Lua counterparts. */
    luaUI_push_widget_ex (L, g->current ? WIDGET (g->current->data) : NULL, TRUE, TRUE);
...
l_dialog_map_widget (lua_State *L)
{
    ...
    g = GROUP (LUA_TO_DIALOG (L, 1));
    w = luaUI_check_widget (L, 2);

    if (w->owner != NULL)
        return luaL_error (
            L, E_ ("Attempt is made to add to a dialog a widget which was already added."));
 ...
 * [info]
 *
 * We don't have properties named "title" and "label" in our API. We always
 * call it "text". This way you don't need to remember what property goes
 * with what widget type.
 *
 * [/info]
 *
 * @attr dialog.text
...
l_dialog_set_text (lua_State *L)
{
    ...
    if (widget_get_state (WIDGET (dlg), WST_MODAL))
        frame_set_title (FRAME (dlg->bg), text);
...
l_dialog_get_text (lua_State *L)
{
    ...
    {
        const char *text = FRAME (dlg->bg)->title;
...
{
    widget_set_state (WIDGET (LUA_TO_DIALOG (L, 1)), WST_MODAL, lua_toboolean (L, 2) != 0);
    return 0;
}
...
l_dialog_destroy (lua_State *L)
{
    ...
    if (!widget_get_state (w, WST_CLOSED) && !widget_get_state (w, WST_CONSTRUCT))
    {
        // In case a programmer tries to close a dialog by calling destroy()
        return luaL_error (L,
                           E_ ("Attempt is made to destroy a dialog which isn't closed (call "
                               "dialog:close() first)."));
    }
 ...
 * [note]
 *
 * Another way to close a dialog is by sending it the "cancel" command (i.e.,
 * @{command|dlg:command "cancel"}). The difference between the two ways is
 * explained in a note in @{git:tests/nonauto/close_current_dialog.lua}.
 *
 * [/note]
 *
 * @method dialog:close
...
l_dialog_get_state (lua_State *L)
{
    ...
    if (widget_get_state (w, WST_CONSTRUCT))
        state = "construct";
    else if (widget_get_state (w, WST_ACTIVE))
        state = "active";
    else if (widget_get_state (w, WST_SUSPENDED))
        state = "suspended";
    else if (widget_get_state (w, WST_CLOSED))
        state = "closed";
    else
        state = "_invalid_";
...
l_dialog_set_compact (lua_State *L)
{
    ...
    dlg->compact = lua_toboolean (L, 2) != 0;
 ...
 * [tip]
 *
 * You can close or cancel a dialog from this event (i.e., by calling
 * @{dialog:close|:close} or @{widget:command|:command 'cancel'}). This lets
 * you "automate", or "auto submit", forms.
 *
 * [/tip]
 *
...
{
    // dialog_switch_list() checks for NULLs, so we too. It may not be necessary.
    if (dlg != NULL)
    {
