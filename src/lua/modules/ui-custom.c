...
#include "lib/tty/tty.h"    // tty_gotoyx() for widget_gotoyx()
...
static int l_custom_set_mouse_capture (lua_State *L);
...
custom_callback (Widget *w, Widget *sender, widget_msg_t msg, int parm, void *data)
{
    ...
    case MSG_DRAW:
    {
        ...
        if (DIALOG (w->owner)->colors != NULL)
            tty_setcolor (DIALOG (w->owner)->colors[DLG_COLOR_NORMAL]);
        else
...
static void
ev_buttons_to_table (lua_State *L, mouse_event_t *event)
{
    lua_newtable (L);
...
static int
push_ev_args (lua_State *L, mouse_event_t *event)
{
 ...
 */
static gboolean
call_abortive_widget_method (Widget *w, const char *method_name, int nargs, gboolean *method_found)
{
...
static void
custom_mouse_callback (Widget *w, mouse_msg_t msg, mouse_event_t *event)
{
    ...
    case MSG_MOUSE_CLICK:
        // First we try "on_mouse_click", then "on_click". See rationale in ldoc for on_click.
        method = widget_method_exists (w, "on_mouse_click") ? "on_mouse_click" : "on_click";
        break;
    case MSG_MOUSE_DRAG:
        method = "on_mouse_drag";
        break;
    ...
    abort = !call_abortive_widget_method (w, method, push_ev_args (Lg, event), NULL);
...
custom_constructor (void)
{
    ...
    w = WIDGET (c);
    widget_init (w, &r, custom_callback, custom_mouse_callback, "Custom");
