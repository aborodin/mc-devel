...
#define GET_INT_FIELD(name)                                                                        \
    ...
    {                                                                                              \
        if (lua_isnumber (L, -1) == 0)                                                             \
            luaL_error (L, E_ ("field '%s' should be numeric, but instead is %s."), #name,         \
                        luaL_typename (L, -1));                                                    \
...
#endif
    else invalid_field_error (L, field);
// clang-format on
}
...
l_statbuf_index (lua_State *L)
{
    ...
    // A special case: the only method we currently support.
    if (STREQ (field, "extract"))
    {
