...
luaMC_checkregex_ex (lua_State *L, int index, int index_flags)
{
    ...
    luaL_error (
        L, E_ ("Unrecognized format for regex. A string, table, or compiled regex is expected."));
...
l_compile (lua_State *L)
{
    ...
    // Guard against the user doing `compile(".", "u")` instead of `compile {".", "u"}`.
    luaMC_checkargcount (L, 1, FALSE);
    return 1;
}
...
push_captures (lua_State *L, const GMatchInfo *match_info)
{
    ...
    // "-1" gives the number of captures.
    count = g_match_info_get_match_count (match_info) - 1;

    // We start from "1". "0" is the whole match.
    for (i = 1; i <= count; i++)
...
eval_cb (const GMatchInfo *match_info, GString *result, gpointer data)
{
    ...
    g_free (whole_match);
    // GLib's manual: "FALSE to continue the replacement process, TRUE to stop it.
    return FALSE;
}
...
split (lua_State *L, gboolean return_table)
{
...

    {
        ...
        count = luaMC_push_argv (L, result, return_table);
        g_strfreev (result);
