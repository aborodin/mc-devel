...
// copied from "src/editor/syntax.c" (where it's not actually used)
...
static int l_edit_add_keyword (lua_State *L);
...
{
    // perchance we haven't been added to a dialog yet.
    if (WIDGET (e)->owner != NULL)
 ...
 * @return *true* unless an error occurred (anything that causes an error message to be shown).
 */
static int
l_edit_load (lua_State *L)
{
     ...
     * @todo:
     *
     * To be really useful, the current file should be recorded in the history
     * so the user can go back ("Back from declaration", bound by default to
     * M-<minus>). This is a must for implementing tags utilities.
     *
     * See editcmd_dialogs.c:editcmd_dialog_select_definition_show() and
...
l_edit_get_line (lua_State *L)
{
    ...
    edit = LUA_TO_EDITBOX (L, 1);
    // on the Lua side we're 1-based.
    line_no = luaL_optlong (L, 2, edit->buffer.curs_line + 1) - 1;
    ...
    // Prevent reporting spurious "\n" on the last line.
    if (finish > edit->buffer.size)
...
l_edit_get_current_char (lua_State *L)
{
    ...
    if (edit->utf8)
    {
        ...
        // "char_length > 0" would be fine. The "> 1" is optimization.
        if (char_length > 1)
        {
 ...
 *
 * An editbox normally fills the whole client area of the editor ("fullscreen"),
 * but it may alternatively be shown inside a resizeable framed box within.
 *
 * [tip]
 *
 * The editor --though not many users are aware of this-- is an
 * [MDI application](https://en.wikipedia.org/wiki/Multiple_document_interface).
 * It may contain more than one editbox. You can see this clearly if you use
 * the `Window / Toggle fullscreen` command from the menu.
 *
 * [/tip]
 *
 * Example:
...
l_edit_bookmark_set (lua_State *L)
{
    ...
    // book_mark_insert() does REDRAW_LINE, but cursor may not be on marked line.
...
l_edit_bookmark_clear (lua_State *L)
{
    ...
    // book_mark_clear() does REDRAW_LINE, but cursor may not be on marked line.
    edit->force |= REDRAW_PAGE;
    edit_update_view (edit);
...
l_edit_bookmark_exists (lua_State *L)
{
    ...
    lua_pushboolean (L,
                     color == -1 ? book_mark_get_topmost_color (edit, line) != 0
...
l_edit_set_cursor_line (lua_State *L)
{
    ...
    if (do_center)
        edit_move_display (edit, line - WIDGET (edit)->rect.lines / 2 - 1);
    edit_move_to_line (edit, line - 1);
    // Why does edit_goto_cmd() do this? Seems unneeded.
    // edit->force |= REDRAW_COMPLETELY;
    edit_update_view (edit);
...
l_edit_set_syntax (lua_State *L)
{
    ...
    if (edit->filename_vpath == NULL)
    {
        ...
        return luaL_error (L,
                           E_ ("Midnight Commander bug: you cannot set a syntax for an edit buffer "
                               "which doesn't have an associated filename."));
    }
 ...
 *
 * Info-short: For stylistic uniformity with @{tty.is_utf8|tty.is_utf8()}
 * only, this was made a method, not a property.
 *
 * @method is_utf8
 */
static int
...
l_edit_set_option (lua_State *L)
{
    ...
    {
        // UI name: "Fake half tabs"
        edit_options.fake_half_tabs = lua_toboolean (L, 2);
    }
    else if (STREQ (option, "expand_tabs"))
    {
        // UI name: "Fill tabs with spaces"
        edit_options.fill_tabs_with_spaces = lua_toboolean (L, 2);
    }
    else if (STREQ (option, "show_numbers"))
    {
        edit_options.line_state = lua_toboolean (L, 2);
        edit_options.line_state_width = edit_options.line_state ? LINE_STATE_WIDTH : 0;
        redraw_editors ();
    }
    else if (STREQ (option, "show_tabs"))
    {
        edit_options.visible_tabs = lua_toboolean (L, 2);
        redraw_editors ();
    }
    else if (STREQ (option, "show_tws"))
    {
        edit_options.visible_tws = lua_toboolean (L, 2);
        redraw_editors ();
    }
    else if (STREQ (option, "show_right_margin"))
    {
        edit_options.show_right_margin = lua_toboolean (L, 2);
        redraw_editors ();
    }
    else if (STREQ (option, "wrap_column"))
    {
        // UI name: "Word wrap line length"
        edit_options.word_wrap_line_length = luaL_checkint (L, 2);
        redraw_editors ();
    }
...
l_edit_get_option (lua_State *L)
{
    ...
    if (STREQ (option, "tab_size"))
        ...
        lua_pushboolean (L, edit_options.fake_half_tabs);
    else if (STREQ (option, "expand_tabs"))
        lua_pushboolean (L, edit_options.fill_tabs_with_spaces);
    else if (STREQ (option, "show_numbers"))
        lua_pushboolean (L, edit_options.line_state);
    else if (STREQ (option, "show_tabs"))
        lua_pushboolean (L, edit_options.visible_tabs);
    else if (STREQ (option, "show_tws"))
        lua_pushboolean (L, edit_options.visible_tws);
    else if (STREQ (option, "show_right_margin"))
        lua_pushboolean (L, edit_options.show_right_margin);
    else if (STREQ (option, "wrap_column"))
        lua_pushinteger (L, edit_options.word_wrap_line_length);
    else if (STREQ (option, "save_position"))
        lua_pushboolean (L, edit_options.save_position);
    else
        luaL_error (L, E_ ("Unknown option name '%s'"), option);
...
l_edit_get_syntax_list (lua_State *L)
{
    ...
    // @FIXME: make that function itself call g_ptr_array_sort(), instead of all its callers doing
    // it?
