...
l_activate (lua_State *L)
{
    ...
    // Standalone: it looks safe (and useful) for Open, but what about Edit and View?
...
expand_format__string (const char *template, Widget *edit_widget, gboolean do_quote)
{
    ...
    while (*p != '\0')
    {
        ...
        else
        {
            ...
            mc_g_string_concat (buf, s);
            g_string_free (s, TRUE);
        }

        // Protect against '%' at end of string.
        if (*p != '\0')
            p++;
    }
 ...
 *    end)
 *
 * For a list of the available tokens, see
 * @{git:usermenu.c|src/filemanager/usermenu.c:expand_format()}.
 *
...
l_expand_format (lua_State *L)
{
    ...
    s = expand_format__string (template, w, !dont_quote);
...
l_name_quote (lua_State *L)
{
    ...
    qs = name_quote (s, quote_percent);
    lua_pushlstring (L, qs->str, qs->len);
