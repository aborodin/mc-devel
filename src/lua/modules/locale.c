...
static int l_format_number (lua_State *L);
...
static void
add_commas_int (char *in, char *out)
{
    char *p, *q;
...
#ifdef HAVE_ENCODING_PRINTF_GROUPING
static int
...
{
    numeric_locale_is_posix =
        STREQ (setlocale (LC_NUMERIC, NULL), "C") || STREQ (setlocale (LC_NUMERIC, NULL), "POSIX");
