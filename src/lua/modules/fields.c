...
static const char *current_field_id = NULL;
static WPanel *current_field_panel = NULL;
...
render_multiplex (file_entry_t *fe, int len)
{
    if (luaMC_get_system_callback (Lg, "fields::render_field"))
    {
        ...
        lua_pushstring (Lg, fe->fname->str);
        ...
        {
            // Borrowed from panel.c:string_file_name()
            static char buffer[MC_MAXPATHLEN * MB_LEN_MAX + 1];
            const char *s;

            s = lua_tostring (Lg, -1);
            g_strlcpy (buffer, s ? s : "", sizeof (buffer));
            // We must always clean up after a successful luaMC_safe_call()
            lua_pop (Lg, 1);  //
            return buffer;
        }
...
sort_multiplex (file_entry_t *a, file_entry_t *b)
{
    ...
    {
        lua_pushstring (Lg, current_field_id);
        lua_pushstring (Lg, a->fname->str);
        luaFS_push_statbuf (Lg, &a->st);
        lua_pushstring (Lg, b->fname->str);
...
l_register_field (lua_State *L)
{
    ...
    panel_field_t field = {
        "", 12, FALSE, J_LEFT, "", "", FALSE, FALSE, NULL, NULL,
    };
    ...
    // title
    field.title_hotkey = luaL_checkstring (L, 2);
    // sort indicator. @FIXME: misnomer.
    field.hotkey = luaL_checkstring (L, 3);
    // @FIXME: "min_size" is a misnomer. It's the _default_ size.
    field.min_size = luaL_checkint (L, 4);
...
{
    // When restarting Lua, clear previously registered fields.
    panel_fields_init ();
...
mc_lua_set_current_field (WPanel *panel, const char *field_id)
{
    ...
    // @todo: is it possible for it to be g_free()'ed while we're holding it? and in the future?
