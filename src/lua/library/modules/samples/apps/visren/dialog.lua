...
local function run_dialog(world, opts)
  ...
  local ipt = ui.Input{opts.input or "", history="visren pattern", expandx=true}
  local template = ui.Input{opts.template or "", history="visren replacement", expandx=true}
  local match_type = ui.Radios{items=search.get_menu()}
  local global = ui.Checkbox{T"Glob&al", checked=opts.global}
  local ignore_case = ui.Checkbox{T"&Ignore case", checked=opts.ignore_case}
