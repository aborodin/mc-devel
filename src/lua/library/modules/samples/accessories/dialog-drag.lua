...
function Mover:on_mouse_down(x, y, ...)
  ...
  self.dialog:fixate()
  -- In the past, before mc's widgets started using the new mouse API (see
  -- commit 51d0783bf), they were effectively misbehaving and we had to do
  -- `self:focus()` here to prevent mouse events from landing in other widgets
  -- when the mouse was moved fast. This isn't needed anymore. See explanation
  -- in old branches if you're curious.
