...
Tells MC to re-parse the format string.

Background-story:

On the C side, a panel keeps a list (WPanel.format) of structures
(format_e) describing the fields it displays and how to display them
(pointers to the rendering functions). This list is re-built whenever
the "format" string is parsed.

If you register a field that shadows one of the build-in names (e.g.,
name, size), you need to trigger this function so that the pointers
mentioned above are updated to point to the Lua rendering function. But
you only need to do this if the panels **have come up already.** (Since
users usually register fields at an earlier stage, they can be oblivious
to this function, which is why we don't advertise it.)

]]

function fields._reparse_format_string()

  -- To re-build the internal list mentioned above, we re-set the "list_format"
  -- property. This property (and a few others) calls the C function
  -- set_panel_formats(), which re-parses the format string.
  ...
  local function reparse_format(pnl)
    if pnl then
      ...
      -- second call.
      pnl.list_format = pnl.list_format
      pnl.list_format = pnl.list_format
    end
  end
...
-- removed, so we need the C side to re-build its internal list.
--
-- Note: You need to require() this module in _bootstrap.lua or else, if
-- no field is registered, this module won't be autoloaded and this handler
-- won't be installed.
--
event.bind("core::after-restart", fields._reparse_format_string)
