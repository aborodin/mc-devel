...
// global because luafs-gc.c needs it.
struct vfs_class vfs_luafs_ops;
...
handle_returned_pair__as_statbf (struct stat *buf)
{
    ...
    /* This shouldn't happen: The Lua side already raises an exception if a filesystem
     * provides invalid value. */
    g_assert (sb != NULL);
...
luafs_open (const vfs_path_t *vpath, int flags, mode_t mode)
{
    if (luaMC_get_system_callback (Lg, "luafs::open"))
    {
        ...
        // Lua name: mc_flags
        lua_pushinteger (Lg, flags);
        // Lua name: creation_mode
        lua_pushinteger (Lg, mode);
        // Lua name: posix_flags
        lua_pushinteger (Lg, NO_LINEAR (flags));
...
{
    static struct vfs_dirent dent = { NULL };
    ...
    if (luaMC_get_system_callback (Lg, "luafs::readdir"))
    {
        ...
        if (luaMC_safe_call (Lg, 1, 3))
        {
...

            {
                ...
                vfs_dirent_init (&dent, d_name, d_ino, DT_UNKNOWN);
...
static int
luafs_utime (const vfs_path_t *vpath, mc_timesbuf_t *times)
{
    if (luaMC_get_system_callback (Lg, "luafs::utime"))
    {
        ...
        if (times != NULL)
        {
            ...
            lua_pushi (Lg, (*times)[1].tv_sec);
            lua_pushi (Lg, (*times)[0].tv_sec);
#else
 ...
 */
static gboolean
dummy_event_handler (const gchar *event_group_name, const gchar *event_name, gpointer init_data,
                     gpointer data)
{
