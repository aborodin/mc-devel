...
fish_read (void *fh, char *buffer, size_t len)
{
    ...
    n = fish_command_v (me, super, WANT_STRING, fish_super->scr_get,
                        "FISH_FILENAME=%s FISH_STARTOFFSET=%" PRIuMAX " FISH_CHUNKSIZE=%zu;\n",
    ...
    if (fish_file->total < 0)
    {
        ...
        fish_file->total = (off_t) strtol (reply_str, NULL, 10);
#else
        fish_file->total = (off_t) g_ascii_strtoll (reply_str, NULL, 10);
#endif
    }
    ...
    while (TRUE)
    {
        n = read (fish_super->sockr, buffer + ret, len - ret);
        if (n < 0)
        {
            if (errno == EINTR)
                continue;
    ...
    {
        /* cases 2 and 3 */
        n = read (fish_super->sockr, reply_str, remain);
...
fish_write (void *fh, const char *buffer, size_t len)
{
    ...
    n = fish_command_v (me, super, WAIT_REPLY,
                        ...
                        "FISH_FILENAME=%s FISH_FILESIZE=%zu;\n", fish_file->file_name, len);
